...
#colouring partition spheres 
 ...
 #provides the numbers of the parts
  ...
  for (i in 1:length(PartLevels)){
    spheres3d( RefCoords[(Part[[i]]),]*1000, col=Colours[i], lit=TRUE, radius = 900, asp=F)
...
PRTT=gridPar(pt.bg="hotpink", pt.size = 0.5)

open3d()
plotRefToTarget(MeanShape,PCA_mean$pc.shapes$PC1max, method="vector", mag = 1, gridPars = PRTT)



for (i in 1:length(PartLevels)){
    spheres3d( MeanShape[(Part[[i]]),], col=Colours[i], lit=TRUE, radius = 0.003, asp=F)
    
  }


shade3d (Meanwarp_Dviv, color="grey", alpha=0.1 )
...
PRTT=gridPar(pt.bg="green", pt.size = 0.5)



plotRefToTarget(ref, PCA_mean$pc.shapes$PC1max, method="vector", mag=0.6,gridPars = PRTT)
...
dimnames(GPA_Allmean$coords)[[3]]==speclist$Specimen

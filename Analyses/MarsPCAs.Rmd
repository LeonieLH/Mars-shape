...
plot(PCA$pc.scores[,1], PCA$pc.scores[,2], asp=T, xlab= pc1lab, cex.axis=1.3, cex.lab=1.3,
ylab= pc2lab, pch = fos.gp, cex = 1.5, bg = col.gp, bty="n") # plots PCA

# Add coloured polygons for the following clades:
clades <- levels(classifiers$Cladename)[c(1,2,4,7,11)]
for(i in clades){
tmp <- which(classifiers$Cladename == i & classifiers$Fossil == "no")
EP <- rownames(PCA$pc.scores[tmp,])[chull(PCA$pc.scores[tmp, c(1,2)])] # find outer edge points
polygon(PCA$pc.scores[EP,c(1,2)], col=adjustcolor(cols[which(names(cols) == i)],
alpha.f = 0.3), border=cols[which(names(cols) == i)]) # Plot convex hull as polygon
}

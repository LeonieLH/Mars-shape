...
plotRefToTarget(GPA$coords[,,34], GPA$coords[,,33], method="vector")
...
Having trouble with subsetting the landmarks to the partitions
...
#WomCrPart is your part.gp
 ...
 #provides the numbers of the parts
  ...
  for(i in 1:length(PartLevels)){
    Part[[i]]<-which (part.gp[,2] == PartLevels[[i]])
  }
  
  ##colours the spheres for each partition; Replace WomCrRef with any configuraiont e.g. mean specimen
  open3d()
  for (i in 1:length(PartLevels)){
    spheres3d(part.gp[Part[[i]],1], GPA_mean$coords[Part[[i]],3], col=Colours[i], lit=TRUE, radius = 0.2, asp=F)
    
  }
  
  basi <- ind.coords[which(part.gp==2), , ]
  reskull <-ind.coords[which(part.gp==1), , ]

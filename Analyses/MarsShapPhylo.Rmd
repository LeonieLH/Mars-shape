...
write.csv(names(ind.Csize), file="../Data/Raw/Mars_classifier_list.csv", row.names=FALSE, col.names=FALSE)
...
#The PCA transformation ensures that the horizontal axis PC1 has the most variation, the vertical axis PC2 the second-most, and a third axis PC3 the least. Obviously, PC3 is the one we drop. (reason for looking at eigenvalues)
...
#To understand the variation in shape between the marsupial species I will run a Principle Component Analysis (PCA). This analysis 

#### RAN INTO A PROBLEM WITH THIS, WONT SHOWTHE SPECIES NAMES
```{r}
#Run PCA
PCA <- plotTangentSpace(GPA_mean$coords, label=dimnames(GPA_mean$ind.coords)[[3]])

#PCA eigenvalues
PCA$pc.summary$importance

#If there is an obvious problem specimen, find out what number specimen it is on the GPA
dimnames(GPA$ind.coords)[[3]]

plotRefToTarget(GPA$ind.coords[,,6], GPA$ind.coords[,,16], method="vector")

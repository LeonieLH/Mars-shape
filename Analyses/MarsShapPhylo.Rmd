...
#Obtain lmk distances between nose and back of head
Distances=interlmkdist(Data, matrix( c(1,4), ncol = 2, byrow=TRUE))
...
plot(PCA_all$pc.scores[,1]~ log(GPA_AllSpecimens$Csize));text(PCA_all$pc.scores[,1]~ log(GPA_AllSpecimens$Csize), labels=names(GPA_AllSpecimens$Csize))

#FOR CSIZE CHECKING
Csizeorder<-sort(log(GPA_mean$Csize), decreasing=TRUE)
Csizeorder

Csizeorder<-sort(GPA$Csize, decreasing=TRUE)
Csizeorder

#you might need to paste this into the r console
labels = names(GPA_mean$Csize)
plot(PCA_all$pc.scores[,1]~GPA_mean$Csize ); text(PCA_all$pc.scores[,1]~GPA_mean$Csize, labels=names(GPA_mean$Csize))

labels = names(GPA$Csize)
plot(PCA$pc.scores[,1]~GPA$Csize ); 
text(PCA$pc.scores[,1]~GPA$Csize, labels=names(GPA$Csize))
...
Allom<-procD.lm(coords~Csize, logsz=TRUE, data=GPA_AllSpecimens, iter=10000, print.progress=FALSE)
summary(Allom)

summary(procD.lm(coords~Csize,  data=GPA_AllSpecimens, logsz=TRUE,iter=10000, print.progress=TRUE))

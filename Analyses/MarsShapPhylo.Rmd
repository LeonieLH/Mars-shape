...
```{r}
#Entering this code to allow me to see the positive and negative extreme ends of PC2 & 3
...
PCA_m12 <- plotTangentSpace(GPA_Allmean$coords, axis1 = 1, axis2 = 2, label=dimnames(GPA_Allmean$coords)[[3]])
...
plotGMPhyloMorphoSpace(tree_synch, GPA_Allmean$coords, tip.labels = FALSE)

plotGMPhyloMorphoSpace(tree_synch, GPA_Basimean$coords, tip.labels =TRUE)

plotGMPhyloMorphoSpace(tree_synch, GPA_Restmean$coords, tip.labels =TRUE)
...
#Graphs with species' name
par(mfrow=c(3,1))

plot(PC1~Csize, data=PC_Csize)
abline (lm(PC_Csize$PC1~PC_Csize$Csize))
text(PC_Csize$PC1~PC_Csize$Csize, labels=rownames(PC_Csize), col=col.gp)
...
# Making a clade-level colour vector for graphs
...
#Making a short form vector of species names of type “G.sp” for plotting
